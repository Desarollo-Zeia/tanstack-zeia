...
interface WaterBillingComparisonProps {
  sedeId: number
}
...
const CHARGE_STYLES: Record<string, ChargeStyle> = {
  agua_potable: { bar: 'bg-primary', iconBg: 'bg-primary', Icon: Droplets },
  alcantarillado: { bar: 'bg-warning', iconBg: 'bg-warning', Icon: Waves },
}

// Colores extra para codigos de cargo no mapeados (evitan repetir colores en la misma tarjeta)
const EXTRA_COLORS = ['bg-success', 'bg-danger', 'bg-text-muted', 'bg-primary-hover', 'bg-text-secondary']
...
function getWaterConsumption(data: WaterBillingCalculateResponse | undefined): number {
  if (!data) return 0
  return data.results.reduce((sum, item) => {
    // Excluir alcantarillado (volumen derivado del mismo consumo): solo se suma agua potable
    if (item.details && item.details.factor == null) {
...
function useWaterBillingCalculateData(sedeId: number, cycle: WaterBillingCycleItem | null) {
  const startDate = cycle?.start_date ?? null
  const endDate = cycle?.end_date ?? null

  return useQuery({
    queryKey: ['water-billing-calculate', sedeId, startDate, endDate],
    queryFn: () => {
      if (!startDate || !endDate) {
        throw new Error('Missing billing cycle')
      }
      return fetchWaterBillingCalculate(sedeId, startDate, endDate)
    },
    enabled: !!startDate && !!endDate,
  })
}

interface WaterBillingCardProps {
  cycles: WaterBillingCycleItem[]
  selectedCycle: WaterBillingCycleItem | null
  onCycleChange: (cycleId: string) => void
  data: WaterBillingCalculateResponse | undefined
  isLoading: boolean
}

function WaterBillingCardSkeleton() {
  return (
...
function WaterBillingCard({
  cycles,
  selectedCycle,
  onCycleChange,
  data,
  isLoading,
}: WaterBillingCardProps) {
  const [hoveredCode, setHoveredCode] = useState<string | null>(null)

  const selectOptions = cycles.map((cycle) => ({
    value: String(cycle.id),
    label: getWaterCycleLabel(cycle),
  }))
  ...
  const barGroups = useMemo(() => {
    if (!data) return []
    return getWaterBillingTotals(data).map((total) => ({
      ...total,
      segments:
        total.amount > 0
          ? data.results
  ...
  const totals = useMemo(() => (data ? getWaterBillingTotals(data) : []), [data])
          ...
          <>
            ...
            <ul className="space-y-3">
              {data.results.map((item) => {
                ...
                const detailLine = getWaterChargeDetailLine(item)
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-text-muted">
                        {detailLine ?? 'Sin tarifa configurada'}
                      </p>
                    </div>
                    <span
                      className={cn(
...
interface WaterSavingsBannerProps {
  leftData: WaterBillingCalculateResponse | undefined
  rightData: WaterBillingCalculateResponse | undefined
  leftCycle: WaterBillingCycleItem | null
  rightCycle: WaterBillingCycleItem | null
  isLoading: boolean
  hasCycles: boolean
}

function WaterSavingsBanner({
  leftData,
  rightData,
  leftCycle,
  rightCycle,
  ...
  const leftTotals = getWaterBillingTotals(leftData)
  const rightTotals = getWaterBillingTotals(rightData)
  ...
  const leftConsumption = getWaterConsumption(leftData)
  const rightConsumption = getWaterConsumption(rightData)
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-text-muted">
            Comparando {getWaterCycleLabel(leftCycle)} - {getWaterCycleLabel(rightCycle)}
          </p>
        </div>
      </div>
...
export function WaterBillingComparison({ sedeId }: WaterBillingComparisonProps) {
  ...
  const { data: leftData, isLoading: isLoadingLeft } = useWaterBillingCalculateData(sedeId, leftCycle)
  const { data: rightData, isLoading: isLoadingRight } = useWaterBillingCalculateData(
    sedeId,
    rightCycle
  )
