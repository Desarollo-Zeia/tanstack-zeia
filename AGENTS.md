...
| Consumo Tarifario Agua | `/energia/water/dashboard/tarifario` | Filter Bar (solo sede), Tabs `?tab=` (Mensual = default \| Comparador). Tab Mensual: gráfica + desglose lado a lado (`WaterMonthlyBillingChart` con valores + `S/` encima de cada barra + `WaterMonthlyBillingDetail` con línea de factor `×` en alcantarillado y `Sin tarifa configurada` si `details` es null), `WaterMonthlyBillingHero` abajo (total + delta vs mes anterior + costo/día + ranking + pico). Tab Comparador: `WaterBillingComparison` (Split View por ciclo + banner ahorro), `WaterBillingCycleTable` (factor de descarga + días + ciclo + tuberías vía `water/consumption-cycle/detail`), `WaterBillingDetailTable` (ciclo `is_current` de `water/billing-cycles` + `water/billing-calculate`) |
